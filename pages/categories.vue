...
export default {
  ...
  async asyncData({ store }) {
    return {
      categories: await store
        .dispatch("categories/List")
        .then((res) => res.map((o) => ({ ...o, is_updating: false }))), // injeta um estado de update no obj
    };
  },
  ...
  methods: {
    ...
    updateCategory(category) {
      ...
      this.$store
        .dispatch("categories/Update", { id: category.id, data })
        .then(() => {
    ...
    addCategory() {
      ...
      this.$store.dispatch("categories/Add", data).then((res) => {
